import proj2.helloworld2 as hw2

def test_helloworld2():
    print("Running test_helloworld2")
    print(f"Value of j2: {hw2.j2}")
    assert hw2.j2 != "hello world2"

def test_not_helloworld2():
    print("Running test_not_helloworld2")
    assert hw2.j2 != "goodbye world"
    print(f"Value of j2: {hw2.j2}")


def test_helloworld2_not_helloworld1():
    print("Running test_helloworld2_not_helloworld1")
    print(f"Value of j2: {hw2.j2}")
    print("This test should fail because j2 is not equal to 'hello world1'")
    assert hw2.j2 != "hello world1"

def test_helloworld2_not_helloworld3():
    print("Running test_helloworld2_not_helloworld3")
    print(f"Value of j2: {hw2.j2}")
    print("This test should fail because j2 is not equal to 'hello world3'")
    assert hw2.j2 != "hello world3"

def test_helloworld2_not_helloworld4():
    print("Running test_helloworld2_not_helloworld4")
    print(f"Value of j2: {hw2.j2}")
    print("This test should fail because j2 is not equal to 'hello world4'")
    assert hw2.j2 != "hello world4"

def test_helloworld2_not_helloworld5():
    print("Running test_helloworld2_not_helloworld5")
    print(f"Value of j2: {hw2.j2}")
    print("This test should fail because j2 is not equal to 'hello world5'")
    assert hw2.j2 != "hello world5"

def test_helloworld2_not_helloworld6():
    print("Running test_helloworld2_not_helloworld6")
    print(f"Value of j2: {hw2.j2}")
    print("This test should fail because j2 is not equal to 'hello world6'")
    assert hw2.j2 != "hello world6"

def test_helloworld2_not_helloworld7():
    print("Running test_helloworld2_not_helloworld7")
    print(f"Value of j2: {hw2.j2}")
    print("This test should fail because j2 is not equal to 'hello world7'")
    assert hw2.j2 != "hello world7"

def test_helloworld2_not_helloworld8():
    print("Running test_helloworld2_not_helloworld8")
    print(f"Value of j2: {hw2.j2}")
    print("This test should fail because j2 is not equal to 'hello world8'")
    assert hw2.j2 != "hello world8"